...
async function chatFetch(prompt: string, temperature: number, maxTokens: number, apiKey: string, url: string, model: string): Promise<string | null> {
  if (!apiKey) return null
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS)
  try {
...
async function anthropicFetch(prompt: string, temperature: number, maxTokens: number, apiKey: string, model: string): Promise<string | null> {
  if (!apiKey) return null
  const controller = new AbortController()
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS)
  try {
...
async function callAI(prompt: string, temperature: number, maxTokens: number, config: AIConfig): Promise<string | null> {
  switch (config.provider) {
    case 'gemini': return geminiFetch(prompt, temperature, maxTokens, config.key)
    case 'openai': return chatFetch(prompt, temperature, maxTokens, config.key, OPENAI_API_URL, OPENAI_MODEL)
    case 'nvidia': return chatFetch(prompt, temperature, maxTokens, config.key, NVIDIA_API_URL, NVIDIA_MODEL)
    case 'anthropic': return anthropicFetch(prompt, temperature, maxTokens, config.key, ANTHROPIC_MODEL)
    default: return null
  }
}
...
export async function analyzeClause(before: string, after: string, config?: AIConfig): Promise<ClauseAnalysis> {
...
export async function generateSummary(clauses: ClauseAnalysis[], config?: AIConfig): Promise<SummaryResult> {
  ...
  const raw = config ? await callAI(`Summarize the following contract analysis into 3-5 bullet points:\n\n${JSON.stringify(clauses, null, 2)}\n\nRespond in JSON format:\n{\n  "summary": "3-5 bullet point executive summary"\n}`, 0.3, 512, config) : null
  if (raw) {
...
function generateLocalAnalysis(before: string, after: string): ClauseAnalysis {
  ...
  let summary: string
  if (beforeLen === 0 && afterLen > 0) summary = `A new clause was added (${afterLen} characters).`
  else if (beforeLen > 0 && afterLen === 0) summary = `An existing clause was removed (${beforeLen} characters).`
  else if (before === after) summary = 'No material change detected between these clauses.'
  else if (diff < -200) summary = `The clause was significantly expanded (from ${beforeLen} to ${afterLen} characters).`
  else if (diff > 200) summary = `The clause was significantly shortened (from ${beforeLen} to ${afterLen} characters).`
  else summary = 'The clause was modified. Review the changes carefully.'
