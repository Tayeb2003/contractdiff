...
export async function handleUpload(req: Request): Promise<Response> {
  try {
    ...
    if (!allowed.includes(file.type)) return json({ error: 'Only PDF, DOCX, and TXT files are allowed' }, 400)
...
export async function handlePaste(req: Request): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
    const body = await parseBody<{ title: string; content: string }>(req)
    validate(body, { content: { required: true, message: 'Content is required' }, title: { max: 500 } })
    const id = crypto.randomUUID()
    await db.execute({
      sql: 'INSERT INTO documents (id, user_id, filename, original_name, content, doc_type) VALUES (?, ?, ?, ?, ?, ?)',
      args: [id, userId, `${id}.txt`, body.title || 'pasted-text', body.content, 'text/plain'],
    })
    return json({ id, originalName: body.title || 'pasted-text', docType: 'text/plain', contentLength: body.content.length }, 201)
  } catch (err) {
...
export async function handleListDocs(req: Request): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
...
export async function handleDeleteDoc(req: Request, id: string): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
    const doc = await db.execute({ sql: 'SELECT id FROM documents WHERE id = ? AND user_id = ?', args: [id, userId] })
    if (doc.rows.length === 0) return json({ error: 'Document not found' }, 404)
    const linked = await db.execute({ sql: 'SELECT id FROM analyses WHERE user_id = ? AND (doc_a_id = ? OR doc_b_id = ?)', args: [userId, id, id] })
    const analyses = linked.rows as any[]
