...
export async function handleCreateAnalysis(req: Request): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
    const body = await parseBody<{ docAId: string; docBId: string }>(req)
    ...
    const docA = docAResult.rows[0] as any
    const docB = docBResult.rows[0] as any
    ...
    processAnalysis(analysisId, docA.content as string, docB.content as string, config).catch((err) => {
      logger.error('Background analysis failed', { analysisId, error: (err as Error).message })
    })
...
async function processAnalysis(analysisId: string, contentA: string, contentB: string, config: AIConfig) {
  try {
    ...
    for (const section of changedSections) {
      ...
      await db.execute({
        sql: `INSERT INTO clause_diffs (id, analysis_id, clause_text_before, clause_text_after, plain_english_summary, favors, severity)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
        args: [clauseId, analysisId, section.before, section.after, result.plain_english_summary, result.favors, result.severity],
      })
    ...
    logger.error('Analysis processing failed', { analysisId, error: (err as Error).message })
    await db.execute({ sql: 'UPDATE analyses SET status = ? WHERE id = ?', args: ['failed', analysisId] })
  }
}

export async function handleListAnalyses(req: Request): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
...
export async function handleGetAnalysis(req: Request, id: string): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
    const result = await db.execute({
    ...
    const analysis = result.rows[0] as any
    const clauses = await db.execute({ sql: 'SELECT * FROM clause_diffs WHERE analysis_id = ?', args: [id] })
    return json({
      analysis: {
        id: analysis.id,
...
export async function handleDeleteAnalysis(req: Request, id: string): Promise<Response> {
  try {
    const { userId } = await requireAuth(req)
    const result = await db.execute({ sql: 'SELECT id FROM analyses WHERE id = ? AND user_id = ?', args: [id, userId] })
    if (result.rows.length === 0) return json({ error: 'Analysis not found' }, 404)
