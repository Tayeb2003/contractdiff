...
export async function generateToken(userId: string): Promise<string> {
  ...
  const payload = b64url(JSON.stringify({ userId, exp: Math.floor(Date.now() / 1000) + 86400 * 7 }))
...
export async function verifyToken(token: string): Promise<{ userId: string; exp: number } | null> {
  try {
    ...
    const key = await hmacKey()
    const valid = await crypto.subtle.verify('HMAC', key, b64urlToBuf(parts[2]), new TextEncoder().encode(`${parts[0]}.${parts[1]}`))
